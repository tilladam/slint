...
impl NodeEditorBackground {
    ...
    fn regenerate_link_paths(self: Pin<&Self>) {
        ...
        // Update the structured link paths model
...
impl Item for NodeEditorOverlay {
    ...
    fn input_event_filter_before_children(
        ...
        // Process any pending events (node clicks for selection)
    ...
    fn render(
        ...
        // Process any pending events (node clicks for selection)
        // This ensures events are processed even without input events
        self.process_pending_events();
...
impl NodeEditorOverlay {
    ...
    /// Process pending events (node clicks for selection)
    /// This is called from render() to ensure events are processed even without input events
    fn process_pending_events(self: Pin<&Self>) {
