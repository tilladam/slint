...
struct NodeEditorState {
    ...
    // NOTE: links BTreeMap removed - now owned by NodeEditorBackground
}
...
impl Item for NodeEditorBackground {
    ...
    fn render(
        ...
        if needs_update {
            ...
            // Drop the borrow if we didn't enter the update block
            drop(state);
        }
...
impl NodeEditorBackground {
    /// Process pending reports for links, pins, and nodes
            ...
            for link_str in batch.split(';') {
                ...
                if parts.len() >= 4 {
                    if let (Ok(id), Ok(start_pin), Ok(end_pin), Ok(color_argb)) = (
                        ...
                        state.links.insert(id, LinkRecord { start_pin_id: start_pin, end_pin_id: end_pin, color });
                        needs_regeneration = true;
                    }
                }
            }
    ...
    fn regenerate_link_paths(self: Pin<&Self>) {
    ...
    pub fn find_link_at(self: Pin<&Self>, position: LogicalPoint, hover_distance: f32) -> i32 {
        let zoom = self.zoom();
        let state = self.data.state.borrow();
...
pub struct NodeEditorOverlay {
    ...
    // - reporting_link_id, reporting_link_start_pin_id, reporting_link_end_pin_id, reporting_link_color
    // - link_reported callback
    // - pending_links_batch, pending_deleted_link_ids
    // - link_positions_data, link_bezier_paths, link_positions_changed
...
impl Item for NodeEditorOverlay {
    ...
    fn render(
        ...
        // NOTE: Link regeneration during drag removed - now handled by NodeEditorBackground
...
impl NodeEditorOverlay {
    ...
    fn process_pending_reports(self: Pin<&Self>) {
        ...
        // NOTE: Link reporting code removed - links are now managed by NodeEditorBackground
        ...
        // NOTE: Link regeneration removed - now handled by NodeEditorBackground
    }
